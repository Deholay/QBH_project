...
def HMM_DP_find_closest_song(query_path, target_file, transition_matrix_folder):
    """
    ...
    query_diff = process_audio_to_query_diff(query_path)
    ...
    # 3. 取 HMM 分數前 20 名
    top_20_targets = sorted(hmm_scores.keys(), key=lambda x: hmm_scores[x], reverse=True)[:60]
...
query_path = r"C:\Users\mrjac\Desktop\丁建均老師信號處理專題\QBH_project\hummingdata\20\20lugo_浪人情歌.wav"
...
result = find_closest_song(query_path, target_file, transition_matrix_folder)
print(result)                      

# for audio_file, best_match, final_score in results:
#     print(f"Audio File: {audio_file}, Best Match: {best_match}, Final Score: {final_score:.4f}")